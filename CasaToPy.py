...
for r in range(1, n): 
	...
	if scale == "Counts":
		...
		for n in range(namesRaw.index("CPS"), namesRaw.index("Background CPS")-1):
			ree = names[n]
			names[n] = ree.replace(names[n], names[n]+"_1")
	...
	print("This is the data being plotted:")
	print(names)
	bec = 0
	fig, ax = plt.subplots()
	
	for x in range(0, len(points)):
		if x in range(namesRaw.index("Counts"), namesRaw.index("Background")-1): #just a hacky way of making sure the points relating to the deconvolution (always between counts and background) are the only ones filled in
